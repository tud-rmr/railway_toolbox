...
%% Create straight shaped track

L = 1000; % in m
v_max = 160; % in km/h
map_rot_angle = 30; % in deg

track_map = generateStraigthTrack(1,v_max,L);

%% Plots

d_train = linspace(0,1*sum(track_map.length),100);
[track_id,rel_position,~,abs_postion,t,curv,radii,speed_limit] = calcTrackProperties(d_train,track_map(1,:).ID,0,track_map,map_rot_angle,1);
